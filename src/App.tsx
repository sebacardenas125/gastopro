...
const Card = ({ className = "", children }: any) => (
  <div className={cx("rounded-2xl border border-white/40 bg-white/90 backdrop-blur-xl shadow text-slate-900 dark:bg-slate-900/70 dark:border-slate-700 dark:text-slate-100", className)}>
    {children}
  </div>
);
...
function HelpWidget() {
  ...
  const send = () => {
    if (!name || !email || !msg) return alert("Completa los campos");
      ...
      <Modal open={open} onClose={() => setOpen(false)} title="¿Necesitas ayuda?">
        <div className="grid gap-2">
          ...
          <textarea
            ...
            className="w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-indigo-500 dark:bg-slate-800 dark:text-slate-100 dark:border-slate-700"
          />
          ...
          <div className="text-xs text-slate-600 dark:text-slate-300">Tiempo de respuesta: 24–48 h hábiles</div>
        </div>
      </Modal>
    </>
  );
...
export default function ExpenseTracker() {
  ...
  const [type, setType] = useState<"income" | "expense">(presets.type);
  ...
  const [fromAcc, setFromAcc] = useState<string>(accounts[0]?.id || "cash");
  const [toAcc, setToAcc] = useState<string>(accounts[1]?.id || "debit");
  const [transferAmt, setTransferAmt] = useState<number | string>(0);
  ...
  const fmt = useMemo(
    () =>
      new Intl.NumberFormat(currency === "CLP" ? "es-CL" : currency === "USD" ? "en-US" : "de-DE", {
        style: "currency",
        currency,
  ...
  const add = (e?: React.FormEvent) => {
    ...
    if (recurring) {
      setTemplates((p) => [...p, { id: uid(), type, category: cat, amount: n, note, accountId: acc }]);
      setRecurring(false);
    }
  };
  ...
  }, [month, templates]); // eslint-disable-line

  const totalIn = inMonth.filter((i) => i.type === "income" && i.category !== "transfer").reduce((a, b) => a + b.amount, 0);
  const totalOut = inMonth.filter((i) => i.type === "expense" && i.category !== "transfer").reduce((a, b) => a + b.amount, 0);
  ...
  const history = useMemo(() => {
    ...
    for (let i = 5; i >= 0; i--) {
      ...
      const s = new Date(sy, sm, 1),
        e = new Date(sy, sm + 1, 1);
      const inM = items.filter((t) => {
        const td = new Date((t.date || "") + "T00:00:00");
        return td >= s && td < e;
      });
      const inc = inM.filter((t) => t.type === "income" && t.category !== "transfer").reduce((a, b) => a + b.amount, 0);
      const exp = inM.filter((t) => t.type === "expense" && t.category !== "transfer").reduce((a, b) => a + b.amount, 0);
      pts.push({ mes: `${sy}-${String(sm + 1).padStart(2, "0")}`, ingresos: inc, gastos: exp, saldo: inc - exp });
    }
    return pts;
  }, [items, month]); // eslint-disable-line
  ...
  const accountBalance = (id: string) =>
    items.reduce((acc: number, t: any) => {
      const a = t.amount || 0;
      if ((t.accountId || accounts[0]?.id) === id) {
        return acc + (t.type === "income" ? a : -a);
      }
      return acc;
    }, 0);
  ...
  const downloadJSON = () => {
    ...
    a.href = url;
    a.download = `gastopro-${month}.json`;
    a.click();
  ...
  const downloadCSV = () => {
    ...
    const rows = items.map((t: any) =>
      headers.map((h) => (h === "tags" ? `"${(t.tags || []).join(" ")}"` : String(t[h] ?? ""))).join(",")
    );
  ...
  const setBudget = (id: string, val: string | number) => setBudgets((p) => ({ ...p, [id]: Math.max(0, Number(val) || 0) }));
  ...
  const removeAccount = (id: string) => {
    if (items.some((t: any) => t.accountId === id)) return alert("No puedes eliminar una cuenta con movimientos.");
  ...
  const submitTransfer = (e: React.FormEvent) => {
    ...
    const outTx = {
      ...
      type: "expense",
      category: "transfer",
      note: transferNote || `Transferencia a ${toAcc}`,
      amount: n,
      tags: ["transfer"],
      accountId: fromAcc,
    };
    const inTx = {
      ...
      type: "income",
      category: "transfer",
      note: transferNote || `Transferencia desde ${fromAcc}`,
      amount: n,
      tags: ["transfer"],
      accountId: toAcc,
    };
    ...
    <div className={theme === "dark" ? "dark" : ""}>
      <section className="bg-gradient-to-br from-indigo-600 via-sky-500 to-cyan-400 text-white">
        <div className="max-w-6xl mx-auto px-6 py-10">
          ...
          <div className="mt-4 grid grid-cols-1 sm:grid-cols-3 md:grid-cols-4 gap-3 text-sm">
            <Card>
              <div className="p-3">
                <div className="opacity-90">Ingresos</div>
                <div className="text-xl font-semibold">{fmt(totalIn)}</div>
              </div>
            </Card>
            <Card>
              <div className="p-3">
                <div className="opacity-90">Gastos</div>
                <div className="text-xl font-semibold">{fmt(totalOut)}</div>
              </div>
            </Card>
            <Card>
              <div className="p-3">
                <div className="opacity-90">Saldo</div>
                <div className="text-xl font-semibold">{fmt(totalNet)}</div>
              </div>
            </Card>
            <Card>
              <div className="p-3">
                <div className="opacity-90">Proyección</div>
                <div className="text-xl font-semibold">{fmt(projected)}</div>
                <div className="text-[11px] opacity-80">Promedio diario</div>
              </div>
            </Card>
          </div>
        </div>
        ...
        <Card>
          <div className="p-4 flex items-center justify-between gap-3 flex-wrap">
            ...
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <Button variant="neutral" onClick={() => setAccOpen(true)}>
                Administrar
              </Button>
              <Button variant="neutral" onClick={() => setTransferOpen(true)}>
        ...
        <Card>
          <form ref={formRef} onSubmit={add} className="p-4 grid lg:grid-cols-8 gap-3">
            <select
              ...
              className="rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm dark:bg-slate-800 dark:text-slate-100 dark:border-slate-700"
            >
            ...
            <select
              ...
              className="rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm dark:bg-slate-800 dark:text-slate-100 dark:border-slate-700"
            >
              {"alimentos,servicios,transporte,vivienda,salud,entretenimiento,otros,ingresos".split(",").map((c) => (
                <option key={c}>{c}</option>
              ))}
            </select>
          ...
          <div className="px-4 pb-4 grid md:grid-cols-4 gap-3">
            <div className="flex items-center gap-2">
              ...
              <select
                ...
                className="rounded-xl border border-slate-200 bg-white px-2 py-1 text-sm dark:bg-slate-800 dark:text-slate-100 dark:border-slate-700"
              >
            ...
            <div className="flex gap-2 justify-end">
              ...
              <Button variant="neutral" onClick={downloadCSV}>
            ...
            <div className="p-4 h-[300px]">
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={history}>
                  ...
                  <XAxis dataKey="mes" />
                  <YAxis tickFormatter={(v) => fmt(Number(v))} width={80} />
                  <Tooltip formatter={(v: any) => fmt(Number(v))} />
                ...
                {filtered.map((i: any) => {
                  ...
                  return (
                    <tr key={i.id} className="border-t border-slate-200/70 dark:border-slate-700 hover:bg-slate-50/70 dark:hover:bg-slate-800/60">
                      ...
                      <td className="p-2 whitespace-nowrap">{i.type === "income" ? "Ingreso" : "Gasto"}</td>
                      <td className="p-2 whitespace-nowrap">{acc?.name || "—"}</td>
                      <td className="p-2 whitespace-nowrap">{i.category}</td>
                      <td className="p-2 max-w-[28ch] truncate" title={i.note}>
                        {i.note || "—"}
                      </td>
                      <td className="p-2 whitespace-nowrap">
                        {(i.tags || [])
                          .slice(0, 4)
                          .map((t: string) => (
                            <span key={t} className="inline-block text-[11px] px-2 py-0.5 rounded-full bg-slate-100 text-slate-700 mr-1 dark:bg-slate-800 dark:text-slate-200">
                              {t}
                            </span>
                          ))}
                      </td>
                      <td className="p-2 text-right font-medium">{i.type === "expense" ? `- ${fmt(i.amount)}` : fmt(i.amount)}</td>
        ...
        <Card>
          <div className="p-4">
            <div className="flex items-center gap-2 mb-3">
              ...
              <div className="font-semibold">Metas por categoría</div>
            </div>
                ...
                return (
                  <div key={id} className="rounded-xl border border-slate-200/70 dark:border-slate-700 p-3 bg-white/80 dark:bg-slate-900/60">
                    ...
                    <div className="mt-2 flex items-end gap-2">
                      <Input type="number" value={b} onChange={(e) => setBudget(id, e.target.value)} />
        ...
        <div className="space-y-2">
          {accounts.map((a) => (
            <div key={a.id} className="flex items-center gap-2">
              <Input value={a.name} onChange={(e) => renameAccount(a.id, e.target.value)} />
          ...
          <div className="flex justify-between items-center pt-2">
            <Button variant="neutral" onClick={addAccount}>
              Agregar cuenta
            </Button>
      ...
      <Modal open={transferOpen} onClose={() => setTransferOpen(false)} title="Transferir entre cuentas">
        <form onSubmit={submitTransfer} className="grid gap-3">
          ...
          <Input type="number" inputMode="numeric" placeholder="Monto" value={transferAmt} onChange={(e) => setTransferAmt(e.target.value)} />
          <Input placeholder="Nota (opcional)" value={transferNote} onChange={(e) => setTransferNote(e.target.value)} />
          <div className="flex justify-end gap-2">
            <Button variant="neutral" type="button" onClick={() => setTransferOpen(false)}>
              Cancelar
            </Button>
            <Button type="submit">
              <ArrowLeftRight className="w-4 h-4" /> Transferir
