...
export default function ExpenseTracker() {
  ...
  useEffect(() => {
    ...
    const exists = (tpl: any) =>
      inMonth.some(
        (t) => t.date === first && t.type === tpl.type && t.category === tpl.category && t.amount === tpl.amount && t.note === tpl.note && t.accountId === tpl.accountId
      );
  ...
  const submitTransfer = (e: React.FormEvent) => {
    ...
    setToastMsg("🔁 Transferencia registrada");
    setToastOpen(true);
    setTimeout(() => setToastOpen(false), 2500);
  };
        ...
        <div className="max-w-6xl mx-auto px-6 py-10">
          <div className="flex items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-2">
              <select
                value={currency}
                onChange={(e: Sel) => setCurrency(e.target.value as "CLP" | "USD" | "EUR")}
                className="rounded-xl bg-white/90 text-slate-900 px-2 py-1 text-sm"
              >
                {["CLP", "USD", "EUR"].map((c) => (
      ...
      <main className="max-w-6xl mx-auto p-6 space-y-6">
        ...
        <Card>
          <form ref={formRef} onSubmit={add} className="p-4 grid lg:grid-cols-8 gap-3">
            <select
              value={type}
              onChange={(e: Sel) => setType(e.target.value as "expense" | "income")}
            ...
            <select
              value={cat}
              onChange={(e: Sel) => setCat(e.target.value)}
              className="rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm dark:bg-slate-800 dark:text-slate-100 dark:border-slate-700"
            >
            ...
            <select
              value={accountId}
              onChange={(e: Sel) => setAccountId(e.target.value)}
              className="rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm dark:bg-slate-800 dark:text-slate-100 dark:border-slate-700"
            >
              {accounts.map((a) => (
          ...
          <div className="px-4 pb-4 grid md:grid-cols-4 gap-3">
            <div className="flex items-center gap-2">
              ...
              <select
                value={typeFilter}
                onChange={(e: Sel) => setTypeFilter(e.target.value as "all" | "expense" | "income")}
            ...
            <Input
              ...
              value={search}
              onChange={(e: Inp) => setSearch(e.target.value)}
        ...
        {/* Tabla movimientos (responsive con data-label) */}
        <Card>
              ...
              <tbody>
                ...
                {filtered.map((i: any) => {
                  ...
                  return (
                    <tr
                      key={i.id}
                      className="border-t border-slate-200/70 dark:border-slate-700 hover:bg-slate-50/70 dark:hover:bg-slate-800/60"
                    >
                      <td className="p-2 whitespace-nowrap" data-label="Fecha">
                        {i.date}
                      </td>
                      <td className="p-2 whitespace-nowrap" data-label="Tipo">
                        {i.type === "income" ? "Ingreso" : "Gasto"}
                      </td>
                      <td className="p-2 whitespace-nowrap" data-label="Cuenta">
                        {acc?.name || "—"}
                      </td>
                      <td className="p-2 whitespace-nowrap" data-label="Categoría">
                        {i.category}
                      </td>
                      <td className="p-2 max-w-[28ch] truncate" title={i.note} data-label="Detalle">
                        {i.note || "—"}
                      </td>
                      <td className="p-2 whitespace-nowrap" data-label="Tags">
                        {(i.tags || [])
                          ...
                          .map((t: string) => (
                            <span
                              key={t}
                              className="inline-block text-[11px] px-2 py-0.5 rounded-full bg-slate-100 text-slate-700 mr-1 dark:bg-slate-800 dark:text-slate-200"
                            >
                              {t}
                            </span>
                          ))}
      ...
      <Modal open={transferOpen} onClose={() => setTransferOpen(false)} title="Transferir entre cuentas">
        <form onSubmit={submitTransfer} className="grid gap-3">
          <div className="grid grid-cols-2 gap-2">
            <select
              value={fromAcc}
              onChange={(e: Sel) => setFromAcc(e.target.value)}
              className="rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm dark:bg-slate-800 dark:text-slate-100 dark:border-slate-700"
            >
              {accounts.map((a) => (
                <option key={a.id} value={a.id}>
            ...
            <select
              value={toAcc}
              onChange={(e: Sel) => setToAcc(e.target.value)}
              className="rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm dark:bg-slate-800 dark:text-slate-100 dark:border-slate-700"
            >
              {accounts.map((a) => (
                <option key={a.id} value={a.id}>
          ...
          <Input
            ...
            placeholder="Monto"
            value={transferAmt}
            onChange={(e: Inp) => setTransferAmt(e.target.value)}
          />
